// Header - Top bar with swarm stats, speed selector, and settings
import { useState, useEffect, memo, useCallback } from 'react'
import { useSwarmStore } from '../stores/swarmStore'

// Profile definitions matching Settings
...
function HeaderComponent({ onSettingsClick, apiBase }: HeaderProps) {
  ...
  const [speed, setSpeed] = useState<SpeedLevel>(() => {
    if (typeof window === 'undefined') return 2
    const saved = localStorage.getItem('hivemind-speed')
    return saved ? (parseInt(saved, 10) as SpeedLevel) : 2
  })
  ...
  const handleSpeedChange = useCallback(async (newSpeed: SpeedLevel) => {
    if (saving) return
